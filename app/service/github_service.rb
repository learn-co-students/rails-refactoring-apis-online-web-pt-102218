class GithubService 
    ...
    def get_repos 
        repos = Faraday.get "https://api.github.com/user/repos", {}, {'Authorization' => "token #{self.access_token}", 'Accept' => 'application/json'} 
        response = JSON.parse(repos.body) 
        response.map{|repo| GithubRepo.new(repo)}
    end 
